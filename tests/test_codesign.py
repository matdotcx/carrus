...
def test_verify_codesign_dmg(mock_run, mock_dmg):
    ...
    with patch("carrus.core.codesign.DMGMount") as mock_mount:
        mock_mount.return_value.__enter__.return_value.app_path = (
            Path(tempfile.mkdtemp(prefix="carrus_test_")) / "test.app"
        )
...
def test_verify_signature_requirements(mock_verify):
    ...
    mock_verify.return_value = SigningInfo(signed=True, team_id="ABC123", notarized=True)

    success, errors = verify_signature_requirements(
        Path("test.app"), required_team_id="ABC123", require_notarized=True
    )
    assert success
    assert not errors
    ...
    success, errors = verify_signature_requirements(
        Path("test.app"), required_team_id="XYZ789", require_notarized=True
    )
    assert not success
    assert "Team ID mismatch" in errors[0]

    # Test not notarized
    mock_verify.return_value = SigningInfo(signed=True, team_id="ABC123", notarized=False)
    success, errors = verify_signature_requirements(
        Path("test.app"), required_team_id="ABC123", require_notarized=True
    )
    assert not success
...
def test_verify_codesign_logging(mock_app, caplog):
    ...
    with caplog.at_level(logging.DEBUG):
        with patch("carrus.core.codesign.run_command") as mock_run:
            mock_run.side_effect = [("", "", 0), ("", "TeamIdentifier=ABC123", 0), ("", "", 0)]
