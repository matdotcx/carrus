    ...
    async def build(
        ...
        try:
            ...
            with DMGMount(source_path, build_state) as mount:
                ...
                return BuildResult(success=True, output_path=dest_path, build_state=build_state)

        except BuildError as e:
            build_state.add_error(str(e))
            return BuildResult(success=False, build_state=build_state, errors=[str(e)])
        except Exception as e:
            build_state.add_error(f"Unexpected error: {str(e)}")
            return BuildResult(
                success=False, build_state=build_state, errors=[f"Unexpected error: {str(e)}"]
            )
...
class DMGMount:
    ...
    def cleanup(self):
        ...
        if self._mounted:
            try:
                _run_cmd([HDIUTIL_PATH, "detach", self.mount_point, "-force"], "Unmounting DMG")
                self._mounted = False
            except Exception as e:
...
async def build_package(
    ...
    try:
        ...
        if validation_errors:
            ...
            return BuildResult(success=False, build_state=build_state, errors=validation_errors)
        ...
        try:
            ...
            return BuildResult(success=False, build_state=build_state, errors=[error])
        ...
        if build_type == BuildType.APP_DMG:
            return await AppDmgBuilder.build(
                source_path, destination, progress, task_id, build_state
            )
